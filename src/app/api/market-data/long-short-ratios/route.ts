/**
 * GET  /api/market-data/long-short-ratios  — query stored long/short ratio data
 * POST /api/market-data/long-short-ratios  — trigger a CoinGlass long/short ingest
 *
 * GET query params:
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const ratios = await repo.queryLongShortRatios({
      symbol,
      timeframe: tfRaw,
      startTime,
      endTime,
    ...
    return NextResponse.json({ ratios, count: ratios.length });
  } catch (err) {
    const message = err instanceof Error ? err.message : "Database error";
    console.error("[market-data/long-short-ratios] Query error:", message);
